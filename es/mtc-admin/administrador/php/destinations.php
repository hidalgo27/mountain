                    ...
                    <div class="col-lg-12">
                        <div class="row">
                            <div class="section-title-1 tx-center margin-bottom-30 os-animation" data-os-animation="fadeInUp" data-os-animation-delay="0s">
                                <h1>Destino: <?php echo str_replace('-', ' ',ucwords($_GET["destino"])); ?></h1>
                                <!-- <div class="sec-title-div-3"></div> -->
                                <?php 
                                    $destinos = str_replace('-',' ',$_GET["destino"]);
                                    $consulta = "SELECT * FROM tdestinos WHERE nombre = '$destinos'";
                                    $ejecutar_consulta = $conexion->query($consulta);
                                    $reg_destino = $ejecutar_consulta->fetch_assoc();

                                    echo '
                                        <h6 class="color-blue">'.$reg_destino["descripcion"].'</h6>
                                    ';
                                ?>
                            </div>
                        </div>
                ...
                <div class="search-result">
                    ...
                    <div class="row masonry-container">
                        <?php
                            ...
                            $consulta = "SELECT * FROM tpaquetesdestinos
                            LEFT JOIN tpaquetes
                            ON tpaquetesdestinos.idpaquetes = tpaquetes.idpaquetes
                            LEFT JOIN tdestinos
                            ON tpaquetesdestinos.iddestinos = tdestinos.iddestinos
                            WHERE tdestinos.nombre = '$destinos' ORDER BY tpaquetes.duracion";
                            $ejecutar_consulta = $conexion->query($consulta);
